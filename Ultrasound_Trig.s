...
TIMER1_ICR			EQU	0x40032024

;GPIO Registers
GPIO_PORTF_DATA		EQU 0x40025010 ; Access PF2
...
;---------------------------------------------------					
			...
			THUMB
			EXPORT 	My_Timer0A_Handler	;export handler to startup for proper isr
			EXPORT	PULSE_INIT
...
My_Timer0A_Handler
			...
			ldr r0,[r1]
			cmp r0,#0x04		;look whether LED is high or low.
			beq	makeLow			;if high, make it low
			b	makeHigh		;if low, make it high
			
makeLow		mov r0,#0x00
			str r0,[r1]
			ldr r1,=TIMER0_TAILR		;load sec
			mov32 r0,#100000				;for 100ms=100000us
			str r0,[r1]
			b	done
			
makeHigh	mov r0,#0x04
			str r0,[r1]
			ldr r1,=TIMER0_TAILR		;load sec
			mov r0,#15					;for 15us
			str r0,[r1]
...
done		ldr r1,=TIMER0_ICR			;clear interrupt flag
			ldr r0,[r1]
			orr	r0,#0x01
			str r0,[r1]
...
PULSE_INIT	PROC
			...
			LDR R0, [R1]
			ORR R0, R0, #0x04 ; set bit1 for output
			STR R0, [R1]
